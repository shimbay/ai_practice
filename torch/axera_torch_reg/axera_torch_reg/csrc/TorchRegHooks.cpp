...
struct HostAllocator final : at::Allocator {
  ...
  at::DataPtr allocate(size_t nbytes) override {
    ...
    if (nbytes > 0) {
      data = reinterpret_cast<void *>(
          get_method("hostMalloc")(nbytes).cast<ptr_t>());
      TORCH_CHECK(data, "Failed to allocator ", nbytes, " bytes on host.");
    }
    return {data, data, &ReportAndDelete<kHostFreeMethod>,
            at::Device(at::kCPU)};
  }
  ...
  void copy_data(void *dest, const void *src, std::size_t count) const final {
    py::gil_scoped_acquire acquire;
    get_method("hostCopyData")(reinterpret_cast<ptr_t>(dest),
...
class TorchRegGeneratorImpl : public at::CPUGeneratorImpl {
public:
  TorchRegGeneratorImpl(c10::DeviceIndex device_index) {
    device_ = c10::Device(c10::DeviceType::PrivateUse1, device_index);
    key_set_ = c10::DispatchKeySet(c10::DispatchKey::PrivateUse1);
  }
  ~TorchRegGeneratorImpl() override = default;
};

static at::Generator make_torchreg_generator(c10::DeviceIndex device_index) {
  return at::make_generator<TorchRegGeneratorImpl>(device_index);
}

// Default, global generators, one per device.
static std::vector<at::Generator> default_generators;

struct TorchRegHooksInterface : public at::PrivateUse1HooksInterface {
  TorchRegHooksInterface(){};
  ~TorchRegHooksInterface() override = default;
  ...
  bool isPinnedPtr(const void *data) const override {
    py::gil_scoped_acquire acquire;
    return get_method("isPinnedPtr")(reinterpret_cast<ptr_t>(data))
        .cast<bool>();
  }
  ...
  getDefaultGenerator(c10::DeviceIndex device_index) const override {
    static bool flag [[maybe_unused]] = []() {
      ...
      for (auto i = 0; i < deivce_nums; i++) {
        default_generators[i] = make_torchreg_generator(i);
  ...
  at::Generator getNewGenerator(c10::DeviceIndex device_index) const override {
    return make_torchreg_generator(device_index);
  }
};

static bool register_hook_flag [[maybe_unused]] = []() {
  at::RegisterPrivateUse1HooksInterface(new TorchRegHooksInterface());

  return true;
}();

// Device guard registration
struct TorchRegGuardImpl final : public c10::impl::DeviceGuardImplInterface {
  static constexpr c10::DeviceType static_type = c10::DeviceType::PrivateUse1;

  TorchRegGuardImpl() = default;
  explicit TorchRegGuardImpl(c10::DeviceType t) {
  ...
  void uncheckedSetDevice(c10::Device d) const noexcept override {
    py::gil_scoped_acquire acquire;
    get_method("uncheckedSetDevice")(d.index());
  }
  ...
  c10::Stream exchangeStream(c10::Stream s) const noexcept override {
    py::gil_scoped_acquire acquire;
    auto stream_id = get_method("exchangeStream")(s.id(), s.device_index())
                         .cast<c10::StreamId>();
  ...
  void block(void *event, const c10::Stream &stream) const override {
    py::gil_scoped_acquire acquire;
    get_method("block")((int64_t)event, stream.id(), stream.device_index());
  }
  ...
  bool queryStream(const c10::Stream &stream) const override {
    py::gil_scoped_acquire acquire;
    return get_method("queryStream")(stream.id(), stream.device_index())
        .cast<bool>();
  }
  ...
  virtual void synchronizeStream(const c10::Stream &stream) const override {
    py::gil_scoped_acquire acquire;
    get_method("synchronizeStream")(stream.id(), stream.device_index());
  }
   ...
   */
  void synchronizeDevice(const c10::DeviceIndex device_idx) const override {
    py::gil_scoped_acquire acquire;
    get_method("synchronizeDevice")(device_idx);
  }
  ...
  void recordDataPtrOnStream(const c10::DataPtr &data_ptr,
    ...
    py::gil_scoped_acquire acquire;
    get_method("recordDataPtrOnStream")(data_ptr.get(), data_ptr.device(),
                                        stream.id(), stream.device_index());
  }
...
C10_REGISTER_GUARD_IMPL(PrivateUse1, TorchRegGuardImpl);
